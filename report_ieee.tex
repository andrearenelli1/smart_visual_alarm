...
seconds have elapsed (defaults: $\theta\!=\!0.70$, $C_{\min}\!=\!10$\,s);
...
Fig.~\ref{fig:cm} shows the confusion matrices: at $\theta{=}0.70$ false
...
\begin{figure}[t]
  ...
  \begin{minipage}[t]{0.49\columnwidth}
    ...
    \caption{Metrics vs.\ $\theta$. Dashed: default ($\theta{=}0.70$).}
    \label{fig:sweep}
  \end{minipage}
  \hfill
  \begin{minipage}[t]{0.49\columnwidth}
...
rate on live video. A MobileNetV2 candidate was measured at
$1{,}919$\,ms (0.52\,FPS, Table~\ref{tab:v2_efficiency}),
$5.5{\times}$ slower than MobileNetV1 and incompatible with the
1\,s alarm-window requirement, confirming that any accuracy improvement
must come at lower---not higher---computational cost on this hardware.

Future work includes: applying quantization-aware training (QAT) to the
existing MobileNetV1 backbone, which is expected to raise recall
(currently 49.3\,\% per-frame) by recovering the accuracy degraded
during post-training quantisation, at zero additional inference cost;
replacing the backbone with MCUNet \cite{mcunet},
